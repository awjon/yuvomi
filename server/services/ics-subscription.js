...
import {
  checkSSRF as checkSSRFRaw,
  normalizeUrl as normalizeUrlRaw,
  ssrfSafeAgent,
...
// Dünne Wrapper um die geteilte safe-fetch-Schicht, damit die exportierte
// Ein-Argument-Signatur (und der ICS-spezifische Private-Netz-Opt-in) erhalten
// bleibt. webcal:// wird für ICS zu https:// umgeschrieben.
function normalizeUrl(raw) {
  return normalizeUrlRaw(raw, { allowPrivate: isPrivateNetworkAllowed(), allowWebcal: true });
}

async function checkSSRF(urlStr) {
  return checkSSRFRaw(urlStr, { allowPrivate: isPrivateNetworkAllowed() });
}
